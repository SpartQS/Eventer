...
export default function TeamsList({ searchQuery }: TeamsListProps) {
    ...
    const handleInviteMember = (teamId: string) => {
        ...
        if (team) {
            ...
            setEditMode(true)
            setModalOpen(true)
            // Фокусировка поля для ID будет реализована через пропс в TeamModal
        }
    }
    ...
    const handleSaveTeam = (updatedTeam: Team) => {
        ...
        setFilteredTeams(prev => prev.map(t => t.id === updatedTeam.id ? updatedTeam : t))
        toast.success("Команда обновлена")
    }

    const handleDeleteTeamModal = (teamId: string) => {
        setTeams(prev => prev.filter(team => team.id !== teamId))
        setFilteredTeams(prev => prev.filter(team => team.id !== teamId))
        toast.success("Команда удалена")
        handleCloseModal()
    }
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case "active":
                return <Badge className="bg-green-600 hover:bg-green-700 text-white text-xs">Активная</Badge>
            case "inactive":
                return <Badge variant="secondary" className="text-xs">Неактивная</Badge>
            default:
                            ...
                            <div className="flex items-center justify-between pt-3 border-t mt-auto">
                                ...
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="w-40">
                                        <DropdownMenuItem className="flex items-center gap-2 text-xs" onClick={() => handleEditTeam(team)}>
